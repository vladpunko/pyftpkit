...
- When using `caplog`, set `logger="pyftpkit"` in `caplog.at_level(...)`.
...
- Use `pathlib.PurePosixPath` for remote FTP paths and `pathlib.Path` for local paths.
...
- Use `mocker.AsyncMock` for async methods and async attribute patches.
